...
#### 6.1.8.2 尝试添加分支`tryMoveAdjacentBlock`
...
### 6.1.9 [进阶] 活塞响应事件与移动方块源码

#### 6.1.9.1 事件的统一入口`onSyncedBlockEvent`
...
#### 6.1.11.1 活塞头存在的条件判定`canSurvive`
...
### 6.1.13 [进阶] 基于`Extended=False`的活塞收回事件的无头活塞制造办法
...
但在此处，if分支成立(黑曜石不可移动,!isMovable是true)，因直接移除活塞面前的方块，普通方块被移除。
...
### 6.2.3 欺骗活塞添加/取消添加方块事件

#### 6.2.3.1 欺骗活塞取消添加方块事件:推动上限检测
