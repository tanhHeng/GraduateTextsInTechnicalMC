...
#### 6.1.11.2 尝试添加分支`tryMoveAdjacentBlock`
...
### 6.1.12 [进阶] 活塞响应事件与移动方块源码

#### 6.1.12.1 事件的统一入口`onSyncedBlockEvent`
...
在1.20中，只有b36的source属性是`true`，其它都是`false`。因此瞬推发生，粘性活塞前两格的b36会直接变成空气。其余部分就不细说了，前面都讲过了。

### 6.1.14 [进阶] b36相关的实体运算
...
#### 6.1.15.1 活塞头存在的条件判定`canSurvive`
