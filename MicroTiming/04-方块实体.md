...
## 4.3方块实体间的时序
在每个游戏刻（Game Tick, GT）中，会遍历所有需要更新的方块实体（BlockEntity）。其执行由 LevelChunk 中以下两个列表维护：

- pendingBlockEntityTickers: 待处理的方块实体（新增或延迟加入的）。
- blockEntityTickers: 当前正在执行 tick() 的方块实体。
...
不同区块之间的方块实体顺序则取决于区块的加载顺序，即哪个区块先加载，它的方块实体就会先加入 blockEntityTickers。
