...
> 五羊飞写的太棒了，本部分在很多方面借鉴了[五羊飞的文档](https://github.com/kaniol-lck/Minecraft-Note/tree/main/Piston)
...
再将活塞设置成b36方块实体，发出NC更新，PP更新[^3]

[^3]:注意这里NC更新和PP更新并不是同时发生的，实际上是先NC更新再PP更新
...
3. 创建活塞头b36，发出红石粉二阶毗邻更新与PP更新
4. 在破坏方块位置发出红石粉二阶毗邻更新和NC更新
5. 在移动原位置发出NC更新
6. 在活塞头位置发出NC更新
7. 在活塞底座发出PP更新和NC更新。
...
![Animated_BFS](./img/Animated_BFS.gif)[^4]

[^4]:本图作者 Blake Matheny - Transferred from en.wikipedia to Commons.，CC BY-SA 3.0，https://commons.wikimedia.org/w/index.php?curid=1864649
...
拉下拉杆，中心活塞被激活，以-x>+x>+z的顺序更新周围三个活塞，依次压入队列。接下来游戏依次执行-x方向，+x方向，更新+x方向的下一个活塞，添加方块事件，压入队列，在+z方向活塞后。接下来+z方向活塞会被执行，最后执行+x方向的最后一个活塞。

我们实际上可以将每个活塞抽象为节点，将深度为0的活塞看作是根节点。注意，在实际情况中这张图可能是多源的。然后对着抽象出来的“活塞图”模拟多源BFS运行，其子节点加入队列的顺序便是NC更新顺序，换言之由子节点相对于父节点的方向而定。如果子节点已经被访问过则不会再继续访问。
...
![Rail_Replicator](./img/BlockEvent_ObserverBased.png)
...
可以显著地看到，位于推动轴方向的附着方块无掉落动画，因为其被添加到破坏方块列表里了。而位于侧面的附着方块有掉落动画，原因是受到了方块移动产生的更新。

附着方块的掉落同样会产生更新，但是由于附着方块通常会直接掉落而难以二次利用。例如：

![一次性tnt复制](./img/BlockEvent_ItemDuplication_Handy.png)

而幸运的是我们活在1.20.1，所以我们有一种能被推动的附着方块：珊瑚扇

下图中是一种极为流行的地毯复制:

![地毯复制](./img/BlockEvent_ItemDuplication_Carpet.png)
...
![pld](./img/BlockEvent_PushLimitDetect.png)
