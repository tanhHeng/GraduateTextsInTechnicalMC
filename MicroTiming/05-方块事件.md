...
| <center>元件种类</center> | <center>目标方块</center> | <center>操作类型</center> | <center>附带数据</center> |
|----------------------|-----------------------|-----------------------|-----------------------|
| 活塞                   | 坐标                    | 活塞(粘性/普通)             | （伸出`0`，收回`1`，瞬推收回`2`） |活塞朝向(0~６) |
| 音符盒                  | 坐标                    | 音符盒                   | 0                     |0                                  |
...
该方法是活塞与实体交互的核心。它的主要功能是模拟b36伸出或收回时，对路径上的实体产生的推力效果。
其工作流程如下：

   1. 确定移动参数：首先，获取b36在当前游戏刻（tick）的移动方向和精确距离。
   2. 粗略筛选实体：通过计算b36在这一帧内扫过的空间，快速找出所有可能被推动的实体。
...
为了衡量这些由“活塞更新活塞”造成的延迟，我们在这里引入一个概念：深度。

我们规定所有非BE阶段规划的活塞方块事件的深度为0。我们记由深度n的方块事件计划的方块事件的深度为n+1，其中n为一个非负整数。
...
根据5.1章节、5.5章节等，我们可以发现：活塞在执行方块事件时只对其`方块位置`和`方块类型`进行校对。尽管记录了`活塞朝向`，但它只管辖b36的方向。
...
## 5.11 欺骗活塞添加/取消添加方块事件
### 5.11.1 欺骗活塞取消添加方块事件:推动上限检测
