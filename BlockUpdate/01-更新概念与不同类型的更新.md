...
>
> 有关**更新核**的理解在基础部分不作过多解释，且并非所有NC更新都存在更新核。参见[进阶部分](#172-nc更新的调用与响应)。
...
**侦测器是一个特例**。它并没有调用`onBlockAdded`方法，而是直接在`scheduledTick`中先调用`setBlockState`发出PP更新，再调用`updateNeighbors`发出NC更新。因此，侦测器在亮起和熄灭是遵循先PP后NC的顺序。

~~正在寻找为什么`onBlockAdded`没被调用的原因...~~
